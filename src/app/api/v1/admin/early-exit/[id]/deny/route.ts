...
import { rejectEarlyExit } from "@/server/services/early-exit.service";
...
export const POST = withErrorHandler(
  withAdminAuth(async (_req: NextRequest, ctx: unknown) => {
    ...
    const result = await rejectEarlyExit(params.id);
    ...
    if (user?.email) {
      sendEmail({
        to: user.email,
        subject: "Early Exit Request Denied",
        html: `<p>Hi ${user.displayName}, your early exit request has been denied. You remain an active member of your circle.</p>`,
      }).catch((err) => console.error("[early-exit] notify failed:", err));
    }

    await logAuditAction(adminId, "OTHER", "MEMBER", result.memberId, {
      details: { action: "early_exit_denied" },
    });
